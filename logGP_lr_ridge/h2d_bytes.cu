...
*/
	...
	//------------------------------------------------------------------------//
	size_t databytes = N  * BYTSIZE; 

	//float *a_h = (float*) malloc ( N * num_streams * BYTSIZE);
	//float *b_h = (float*) malloc ( N * num_streams * BYTSIZE);
	//float *c_h = (float*) malloc ( N * num_streams * BYTSIZE);

	float *a_h = NULL;
    checkCudaErrors(cudaMallocHost((void **)&a_h, N * num_streams * BYTSIZE));

	float *b_h = NULL;
    checkCudaErrors(cudaMallocHost((void **)&b_h, N * num_streams * BYTSIZE));

	float *c_h = NULL;
    checkCudaErrors(cudaMallocHost((void **)&c_h, N * num_streams * BYTSIZE));
	...
	float *b_d;
	float *c_d;
	cudaMalloc((void**)&a_d, N * num_streams * BYTSIZE);
	cudaMalloc((void**)&b_d, N * num_streams * BYTSIZE);
	cudaMalloc((void**)&c_d, N * num_streams * BYTSIZE);
