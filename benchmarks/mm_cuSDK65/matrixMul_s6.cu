...
void matrixMultiply(int argc, char **argv, int block_size, dim3 &dimsA, dim3 &dimsB)
{
	...
	float *h_A5 = NULL;
	cudaMallocHost((void **) &h_A5, mem_size_A);
	float *h_A6 = NULL;
	cudaMallocHost((void **) &h_A6, mem_size_A);
	...
	float *h_B5 = NULL;
	cudaMallocHost((void **) &h_B5, mem_size_B);
	float *h_B6 = NULL;
	cudaMallocHost((void **) &h_B6, mem_size_B);
	...
	float *h_C5 = NULL;
	cudaMallocHost((void **) &h_C5, mem_size_C);
	float *h_C6 = NULL;
	cudaMallocHost((void **) &h_C6, mem_size_C);
	...
	float *d_B5 = NULL;
	float *d_C5 = NULL;
    cudaMalloc((void **) &d_A5, mem_size_A);
    cudaMalloc((void **) &d_B5, mem_size_B);
    cudaMalloc((void **) &d_C5, mem_size_C);

    float *d_A6 = NULL;
	float *d_B6 = NULL;
	float *d_C6 = NULL;
    cudaMalloc((void **) &d_A6, mem_size_A);
    cudaMalloc((void **) &d_B6, mem_size_B);
    cudaMalloc((void **) &d_C6, mem_size_C);
    ...
    cudaMemcpyAsync(d_A5, h_A5, mem_size_A, cudaMemcpyHostToDevice, streams[4]);
    cudaMemcpyAsync(d_B5, h_B5, mem_size_B, cudaMemcpyHostToDevice, streams[4]);

    cudaMemcpyAsync(d_A6, h_A6, mem_size_A, cudaMemcpyHostToDevice, streams[5]);
    cudaMemcpyAsync(d_B6, h_B6, mem_size_B, cudaMemcpyHostToDevice, streams[5]);
	...
	matrixMulCUDA<32><<< grid, threads, 0, streams[4]>>>(d_C5, d_A5, d_B5, dimsA.x, dimsB.x);
	matrixMulCUDA<32><<< grid, threads, 0, streams[5]>>>(d_C6, d_A6, d_B6, dimsA.x, dimsB.x);
	...
	cudaMemcpyAsync(h_C5, d_C5, mem_size_C, cudaMemcpyDeviceToHost, streams[4]);
	cudaMemcpyAsync(h_C6, d_C6, mem_size_C, cudaMemcpyDeviceToHost, streams[5]);
