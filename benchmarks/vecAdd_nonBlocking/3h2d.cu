		...
		float *c_d)
{
	...
	if(tid < N) {
		...
		c_d[tid + offset] = a_d[tid + offset] + b_d[tid + offset] + j_d[tid + offset];	
	}
}
...
int main( int argc, char **argv)
{
	...
	float *j_h = NULL;
    checkCudaErrors(cudaMallocHost((void **)&j_h, N * num_streams * FLTSIZE));
	...
	cudaMalloc((void**)&j_d, N * num_streams * FLTSIZE);
	...
	for (int i = 0; i < num_streams; i++) {
		...
		cudaMemcpyAsync(&j_d[offset], &j_h[offset],  databytes, cudaMemcpyHostToDevice, streams[i]);
	}
